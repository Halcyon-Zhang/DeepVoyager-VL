...
def finalize_model_grads(
    ...
    if num_tokens is not None:
        ...
        # PP=1: the group is a singleton and the broadcast is a no-op; issuing it
        # anyway lazily creates one NCCL communicator per rank, which hangs on
        # this cluster. Skip it.
        if torch.distributed.get_world_size(group=pp_group) > 1:
