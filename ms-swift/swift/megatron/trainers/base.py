...
class BaseMegatronTrainer(ABC):
    ...
    @staticmethod
    def _warmup_comm_groups():
        # Eagerly run one tiny collective on every Megatron communicator, in the
        # same order on all ranks, right after initialize_megatron. Lazy NCCL
        # communicator init inside the first train_step deadlocks on this
        # cluster (staggered first use across subgroups).
        ...
        for getter, kwargs in (
            ...
            if group is not None and group not in groups:
                groups.append(group)
        for group in groups:
            dist.all_reduce(t.clone(), group=group)
        torch.cuda.synchronize()
        dist.barrier()
        if dist.get_rank() == 0:
            logger.info(f'Warmed up {len(groups)} Megatron communicator groups.')
