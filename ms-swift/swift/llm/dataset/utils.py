...
class PackingDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Packing runs on every rank (input `lengths` is identical everywhere, so
        # results are deterministic and consistent). The original master-compute +
        # broadcast_object_list deadlocks on multi-node: master finishes the
        # broadcast and races into Megatron communicator setup while workers are
        # still blocked in the broadcast. Packing itself takes <1s.
        if True:
            ...
            with tqdm(total=len(lengths), dynamic_ncols=True, desc=desc, disable=not is_master()) as prog_bar:
                finished_workers = 0
